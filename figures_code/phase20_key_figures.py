...
def fig_micrographs(model, device, amp, coco, images_dir, out_dir,
    ...
    fig, ax = plt.subplots(1, 4, figsize=(7.423, 2.418))
    ax[0].imshow(img_c, cmap="gray")
    ax[0].set_title("phase contrast", fontsize=7.5)
    # 100 um scale bar. LIVECell releases each acquisition as four
    # 704 x 520 crops covering 0.875 x 0.645 mm (Edlund et al. 2021), so one
    # pixel is 1.24 um; cropping does not rescale, so the bar is valid
    # whether or not the centre crop is active.
    ...
    ax[0].plot([_x0, _x1], [_y, _y], "-", color="white", lw=2.0,
               solid_capstyle="butt", zorder=5, path_effects=_o)
    ax[0].text((_x0 + _x1) / 2.0, _y - _h * 0.025, r"100 $\mu$m",
               color="white", fontsize=6.8, ha="center", va="bottom",
    ...
    fig.tight_layout()
    # dpi=400, not 220. This is the ONLY figure in the paper with a
    # substantial embedded raster, and savefig dpi is what decides how many
    # pixels of the 704 x 520 source survive into the PDF: matplotlib
    # resamples each imshow to the axes size in DEVICE pixels, which is
    # (axes width in inches) x dpi. At 220 the four panels were stored at
    # 322 x 238, i.e. 208 effective dpi once placed -- below the 300 dpi
    # floor Elsevier and OUP both specify, and invisible to
    # check_figure_type.py, which measures type size and not resolution.
    # 400 stores close to the full source field without exceeding it, so
    # nothing is interpolated up. See check_figure_resolution.py.
    for ext in ("pdf", "png"):
        fig.savefig(os.path.join(out_dir, f"Fig2_micrographs.{ext}"), dpi=400,
                    bbox_inches="tight")
    plt.close(fig)
