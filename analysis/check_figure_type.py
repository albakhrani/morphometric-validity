...
put a 4.9 pt subscript on the page.
    ...
    python check_figure_type.py                  # OUP measure (the target)
    python check_figure_type.py --venue cas      # Elsevier CAS fallback
...
#   CAS  cas-dc.cls  at this document's settings
#   OUP  oup-authoring-template.cls [contemporary,large]:
#        ==PROBE== tw=526.37598pt cw=254.65216pt
VENUE = {"cas": (494.51, 234.88),
         "oup": (526.376, 254.652)}

TEXTWIDTH, LINEWIDTH = VENUE["oup"]
...
def page_width(path: Path) -> float:
    """Artwork width in pt, from /MediaBox. Parsed directly rather than
    shelled out to pdfinfo, which is not on PATH in every environment this
    has to run in -- and a check that cannot run is a check that passes."""
    data = path.read_bytes()
