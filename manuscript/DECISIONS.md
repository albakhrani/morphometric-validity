...
| **Normalized re-sweep found two more residual claims** | Step 2's retraction used a line-based grep, so the sweep was repeated with whitespace collapsed and hyphenated breaks rejoined, over **both trees' source and both compiled PDFs**. It found two live errors the earlier method had left: (1) *"the measurement-optimal setting ... recovers fewer lineage directions than the detection-optimal setting"* — a fourth D4 assertion, false under C2 where both recover 5/6; corrected to "recovers no more lineage directions ... and fails on A172 more decisively". (2) *"Cellpose remains the only source to lose a lineage even there"* — false once panel (b) moved to C2, since connected components also loses one; corrected to "Cellpose loses BT-474 even there, and is the only source to lose a Tier A trajectory". | after the fixes the sweep returns 20 hits, all legitimate, 0 that a line grep would miss |
| **Fact-dependency read — 225 passages** | A read organised by fact, not a search organised by string, because the residual errors in this revision asserted changed facts *in different words*. Six readers covered the Introduction, both exposed Results subsections, Coverage and the atlas re-run, the whole Discussion through the Conclusion, every float caption, and `cover_letter.tex` — the last never read for fact-dependency before. **225 passages read, 101 flagged, 1 false, 20 imprecise, 80 true as written.** | verdict counts are of passages read, not of grep hits |
| **Amplitude claim scoped** | The Discussion said detection accuracy "governs how far the measured mean is biased and how much amplitude survives, and on both the more accurate model was better". Unscoped, that contradicts `body.tex:892` in the same manuscript: *"It is not a proxy for amplitude anywhere (ρ = −0.314, p = 0.006, higher F1 costing amplitude across the whole sweep)."* Same defect class as the ρ = −0.620 opposition-versus-alignment error corrected twice before: a relationship asserted without the scope that makes it true. Now reads "...between the two models compared here the more accurate one was better on bias and on amplitude alike, although within the parameter sweep higher detection F1 costs amplitude." A scope repair, not a hedge. **An earlier brief that called this claim unaffected was wrong, and the reader who flagged it was right.** | quoted from the fresh PDF |
| **Inventory table completed** | The sample-inventory caption promises "every image count used in this work ... and what is computed on it", and the 180-image row omitted the per-lineage BT-474 detection F1 the revision added. The row now lists it. **Every other row audited against the current manuscript:** 4,875 (expert atlas), 1,564 (test-split pool), 1,420 (coverage denominator), 1,419 (panel (a) and the atlas re-run), 1,154 (panel (b)), 150 (operating-point selection), 168→672 (the MCF7 correction) — each count's stated use is accurate and complete. | a completeness claim that is now complete |
| **Direction definition disambiguated** | Table 7's caption said the recovered coefficient must match the expert sign "and whose percentile bootstrap 95% confidence interval excludes zero", where the second "whose" could be read as the *expert* coefficient's interval. Since this defines the recovery criterion, the wrong reading misreads the whole Direction column. Now: "whose **own** percentile bootstrap 95% confidence interval, **that of the recovered coefficient rather than the expert's**, excludes zero." The Methods definition was carrying the same ambiguity in "its" and now says the same thing in the same terms. | both quoted from the fresh PDF |
| **Type 42 conversion — DONE** | Deferred four times, completed here. `pdf.fonttype = 42` set in all six remaining generators (three imported pyplot inside functions, so the rcParam is set at module import instead), and all seven figures regenerated through `recalibrate_figures.py`. **Zero Type 3 font objects across all seven; every figure now Type0/CID.** `Fig5_merged.pdf` and `atlas_comparison.pdf` extract as readable text with **zero `/uni` escapes**, where they previously produced glyph-index mush. Artwork widths 522.8—527.3 pt against a 526.38 pt target, include scales 0.998—1.007. | 9 checkers FAIL 0, 67/67 controls, pages unchanged at 20 and 24 |
| **Type 42 — the tier assertions now demonstrably read real text** | The specific false pass this conversion risked reintroducing. `Fig2_atlas.pdf` extracts 554 characters through the checker's own `extract()`, and **all eleven lineage-name variants** (BV-2, MCF7, SkBr3, A172, BT-474, SH-SY5Y, SK-OV-3, Huh7 and their unhyphenated forms) are found in it. The assertions are running against real text, not passing on garbage. | checked through the checker's own extractor, not a substitute |
| **Type 42 — two problems it surfaced, both handled** | (1) The regeneration wrote a `.png` beside every `.pdf` plus three unused figure pairs into the submission directory, which made all seven `\includegraphics` references **ambiguous** — extensionless resolution is distribution-dependent, so the build could have silently picked a raster. Thirteen stray files removed; `check_filenames` back to 14 references, 0 problems; **both trees rebuilt from clean afterwards**, since the first build ran while the ambiguity existed. (2) `Fig1_architecture` appeared to regress to 4.37 pt minimum type. It had not: the 4.37 pt item is a **single radical glyph** in "q = P/√A", a mathtext size-variant artifact the checker already exempts for `Fig2_atlas`. Under Type 3 that glyph was not exposed as text at all, so the old 7.01 pt reading was itself a false pass. Verified by eye at 700 dpi — the radical prints at full size — then added to `RADICAL_GLYPHS` with that provenance recorded in the source. | the conversion made a long-standing false pass visible |
| **Type 42 — rendered and inspected** | Nine float pages rendered and looked at, not merely checked. Figure 1's pipeline diagram: all boxes legible, no collisions, the radical correct. Figure 5's four panels: axis labels, legends, colorbar and tick labels all clean, no overlap or clipping. Font substitution shifts text metrics and this is the class of defect no checker sees. | the assets a reader receives |
| **The 1,420 traced to data: it is 1,419** | Settled from `release_repo/data/mask_source_comparison_per_image.csv`, not by reasoning: **1,420 image records, 1,419 with an expert measurement**, the single exception being `BV2_Phase_A4_2_02d04h00m_3.tif` — exactly the BV-2 field the manuscript describes. Rows usable for the atlas re-run (expert plus both operating points) = **1,419**. Every quantity the passage reports is measured against expert coefficients and so requires that measurement. The sentence conflated coverage (both settings measure all 1,420) with the comparison's n; it now states both and names the excluded record. | the CSV settled it |
| **The twelve deferred items, judged individually** | Each given a verdict rather than deferred as a batch. **Two are one-clause repairs worth taking in a follow-up:** the sample-inventory row at `body.tex:260`, whose caption promises "every image count used in this work ... and what is computed on it" but omits the new per-lineage BT-474 F1 use of the 180-image sample (contradicts its own completeness claim; my anchor did not match the wrapped row and I did not chase it rather than risk running out mid-close); and Table 7's Direction definition at `:1476`, where the second "whose" can be read as the expert coefficient's interval rather than the recovered one's — an ambiguity I introduced. **The other ten are genuinely minor:** the abstract's "four mask sources ... over 1,419 images" (pre-existing, implies a four-way comparison that is panel (b)'s); `:1216` seven-of-eight sign agreement, arithmetically right but using "recovered" in the looser sense (flagged twice by different readers); `:1235` BV-2 "recovered" where BV-2 is Tier C; `:1292` the historical coverage-versus-direction note, still accurate; `:1697` the 61% amplitude figure, correct for the frozen test sample; `:1763` "for one lineage", where BV-2 also fits the description; `:1481` the panel (b) caveat, now under-informative; `:284` a source-count phrasing predating the revision; and the cover letter at `:36`, which the reader fact-checked clause by clause and found sound. | 20 imprecisions: 6 applied earlier, 2 applied here, 12 judged |
| **Type 42 conversion — STILL OPEN, fourth deferral** | Not started. No room to finish and verify, and a partially converted figure set is worse than none. It is the only outstanding item and it now needs a session that begins with room for it: six generators need `pdf.fonttype = 42`, `recalibrate_figures.py`'s convergence loop rewrites figsizes back into them, `Fig3_mechanism` and `Fig2_micrographs` need the model checkpoint and raw LIVECell images, `figure3_mechanism.py` needs the project root on PYTHONPATH, and verification ends with rendering the affected pages and looking at them. **The paper builds and ships clean without it.** | open |
| **The one false statement, found twice independently** | *"detection-optimal preserves the most trajectory directions"* — a superlative left over from the 6/6 count. Panel (a) is now tied 5/5/5, so no operating point preserves the most. Two readers working different regions found it independently, which is the strongest signal in the report. Corrected to "they recover the same number of trajectory directions, detection-optimal preserves the better lineage ordering". **No grep for the retracted wording could have found this: it never contained a retracted string.** | the method found what the string search could not |
| **Six imprecision repairs applied, one clause each** | (1) and (2) the Introduction and Discussion both said sources "do not lose the same one"; under the new criterion two of three lose the *same* lineage, so both now name the split — both extension settings lose A172, Cellpose loses BT-474. (3) *Coverage* said connected components "is not scored on the measurement columns there"; it is unscored only in panel (a), and carries values throughout panel (b). (4) The C3 sensitivity sentence said "only the weakest source", ambiguous inside a paragraph about three other sources; now names connected-component labeling at its F1 floor of 0.111. (5) Table 7's caption implied panel (c) backs all three scored sources when it shows two. (6) The Discussion said the derived quantity "degrades" across models; with panel (a) tied it does not degrade, it fails to improve. | quantity digits 802 → 804: `0.111` and the `1` of the metric name F1 |
| **Fourteen imprecisions recorded, not applied** | They needed more than a one-clause repair or touched a claim the authors should adjudicate. The two worth their attention: the Discussion's *"it governs how far the measured mean is biased and how much amplitude survives, and on both the more accurate model was better"* — the amplitude half holds between the two models but not along the parameter sweep; and body.tex:1432, which says a comparison is made "over all 1,420 images" where every quantity it then reports is a panel (a) cell and panel (a) is the 1,419-image set. **I did not change that number**, because changing a number requires tracing it to disk and this session had no budget to do so. | flagged for the authors |
| **Type 42 conversion — STILL OPEN** | Not started; no room to finish and verify, and the brief's own rule says a partially converted figure set is worse than none. Unchanged: six generators need the rcParam, `recalibrate_figures.py`'s convergence loop rewrites figsizes back into them, two figures need the model checkpoint and raw images, `figure3_mechanism.py` needs the project root on PYTHONPATH, and verification ends with a visual pass over rendered pages. **The paper builds and ships clean without it, and it is the only outstanding item.** | open |
| **What the normalized sweep proved about method** | Of the hits, exactly **one** was invisible to a line-based grep — in the CAS PDF, whose text layer runs words together. The other residuals were missed not because of normalization but because **Step 2's term list was incomplete**: "recovers fewer" and "only source to lose a lineage" were never searched for. Normalization was necessary and not sufficient; the term list mattered more. Recorded so the lesson is not mis-drawn. | all nine `6/6` hits confirmed to be panel (b)'s two legitimate cells |
| **Normalization made permanent, with controls** | `check_figure_text.py`'s prose scope collapsed whitespace but did not rejoin hyphenated breaks. It now uses the same `norm()` the figure side uses — every non-alphanumeric character stripped — so a phrase wrapping across a newline, a paragraph indent or a hyphenation point cannot hide. **Three negative controls added, 64 → 67**: a forbidden phrase split across a newline must FAIL; one broken at a hyphenation point must FAIL; clean prose must not. Both catching controls fire. Without them the normalizer would only ever have seen contiguous text. | the class fix for `manu- factured`, `sup- ported`, `six of\nsix` |
| **Panel (b) framing confirmed intact (item 2b)** | The corrected sentence does not lean on the uninterpretable result: *"We report that comparison but do not build on it: the subset is defined by connected-component coverage, is biased toward sparse fields, and truncates the crowded end of the range where the dissociation is largest. Its value is corroborative ... not evidential."* All three caveats survive in the compiled PDF. | quoted from the fresh build |
| **Type 42 conversion — STILL OPEN** | Not started again, for the same reason and under the brief's own rule: it cannot be finished and verified in the room available, and a partially converted figure set is worse than none. Unchanged from the previous entry: six generators need the rcParam, regeneration runs through `recalibrate_figures.py` whose convergence loop rewrites figsizes back into the generators, two figures need the model checkpoint and raw images, `figure3_mechanism.py` needs the project root on PYTHONPATH, and the verification includes a visual pass over the rendered pages. **The paper builds and ships clean without it.** | open |
